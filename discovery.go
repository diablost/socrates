...
func Discovery(c chan int) []string {
	...
	log.Printf("byteData:%v", common.ToHex(byteData))
	// remove "0x" and "\x0e" and "$"(0x24)
	newHost, err := hex.DecodeString(strings.TrimRight(strings.TrimLeft(common.ToHex(byteData), "0x"), "0e"))
	if err != nil {
		log.Fatal(err)
	}
	//addrs <- string(strData)
